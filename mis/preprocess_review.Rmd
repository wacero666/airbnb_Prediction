...
```{r}
df_heatmap <- melt(table(review$neighbourhood_group_cleansed, review$review_scores_rating))

ggplot(data = df_heatmap, aes(x = Var1, y = factor(Var2))) + scale_fill_gradient(low = "white", high = "steelblue") + labs(x = "neighbourhood_group_cleansed", y="review") +
  geom_tile(aes(fill = value), color = "white") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
prop_type <- c("Apartment", "Condominium", "Guesthouse", "House", "Townhouse", "Guest suite")
```
...
pl2 <- ggplot(data = df_heatmap, aes(x = Var1, y = factor(Var2))) + scale_fill_gradient(low = "white", high = "steelblue") + labs(x = "property_type", y="review") +
  geom_tile(aes(fill = value), color = "white") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
prop_type <- c("Apartment", "Condominium", "Guesthouse", "House", "Townhouse", "Guest suite")
ggsave("prop_type.png", device = "png", width =5, height=3)
```
```{r}
...
```{r}
hist(review$maximum_nights, main="maximum night")
```
...
```{r}
n_val <- floor(0.2*nrow(review))
n_train <- nrow(review) - n_val
idx <- sample(1:nrow(review), n_train, replace=FALSE)
review_train <- review[idx,]
review_val <- review[-idx,]
```
...
```{r}
review_train <- review_train[,-c(1,9,11,12,19)]
write.csv(review_train, "review_train.csv", row.names = FALSE)
review_val <- review_val[,-c(1,9,11,12,19)]
write.csv(review_val, "review_val.csv", row.names = FALSE)
```
